...
const EmailTemplates = require("../mailer/MailTemplates");
const Mailer = require("../mailer/SendMail");
const GenereteToken = require("../JWT/GenerateJWT");
...
//http://localhost:9091/User/signin
userRouter.route("/signin2").post((req, res) => {
  User.findOne({
    email: req.body.email,
...
userRouter.route("/signupEtudiant").post(async(req, res) => {
  ...
  User.findOne({ email: req.body.email }).exec((err, user) => {
    if (user) {
      res.status(400).json({ message: "Faild! E-mail Alredy In Use !" });
      return;
    } else {
...
userRouter.route("/signupEtudiant").post(async(req, res) => {
  const etudiant =  new User ({
    ...
    dateNaissance: req.body.dateNaissance,
    mdp: bcrypt.hashSync(req.body.mdp, 8),
    confirmMdp: bcrypt.hashSync(req.body.confirmMdp, 8),
  ...
  User.findOne({ cin: req.body.email }).exec((err, user) => {
    if (user) {
      res.status(400).json({ message: "Faild! E-mail Alredy In Use !" });
      return;
    } else {
...
//http://localhost:9091/User/signupEtudiant2
...
userRouter.route("/signupEtudiant2").post(async(req, res) => {
  const etudiant =  new User ({
    nom: req.body.nom,
    prenom: req.body.prenom,
    email: req.body.email,
...
userRouter.route("/signupEtudiant3").post(async (req, res) => {
  const etudiant = new User({
    nom: req.body.nom,
    prenom: req.body.prenom,
    email: req.body.email,
  ...
  User.findOne({ email: req.body.email }).exec(async (err, user) => {
    if (user) {
      ...
      try {
        ...
        const token = GenereteToken({ id: etudiant._id, email: etudiant.email }, "24h");

        const verif_mail_message = await Mailer.sendMail(
          etudiant.email,
          EmailTemplates.SendMailVerification(
...
userRouter.route("/signupClub").post(image.single('profileImage'),(req, res) => {
  const club = new User({
    nom: req.body.nom,
    prenom: req.body.prenom,
    email: req.body.email,
  ...
  User.findOne({ email: req.body.email }).exec((err, user) => {
    if (user) {
      res.status(400).json({ message: "Faild! Email Alredy In Use !" });
      return;
    } else {
      club.save((err, user) => {
        if (err) {
...
//http://localhost:9091/User/signupClub
userRouter.post("/signupClub2",  (req, res) => {
  const club = new User({
    nom: req.body.nom,
    prenom: req.body.prenom,
    email: req.body.email,
